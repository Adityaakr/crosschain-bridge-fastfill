import { createWalletClient, createPublicClient, http, parseUnits, encodePacked, keccak256, decodeEventLog } from 'viem'
import { privateKeyToAccount } from 'viem/accounts'
import { arbitrumSepolia, baseSepolia } from 'viem/chains'

// WORKING AUTOMATIC STXN BRIDGE - FIXED EVENT DETECTION
...
export interface WorkingAutomaticSTXNTransaction {
  hash: string
  chain: 'arbitrum' | 'base'
...
export class WorkingAutomaticSTXNCallBreakerBridge {
  ...
  // Extract deposit info from transaction receipt
  private async extractDepositInfo(txHash: string) {
    ...
    if (escrowLogs.length === 0) {
      throw new Error('No escrow logs found in transaction')
    }
    
    // Decode the DepositRequested event
    const depositLog = escrowLogs[0]
    ...
    try {
      ...
      return {
        ...
        minReceive: decoded.args.minReceive,
        feeCap: decoded.args.feeCap,
        targetChainId: decoded.args.targetChainId,
        targetToken: decoded.args.targetToken
      }
    } catch (error) {
      // If decoding fails, extract manually from raw data
      const depositId = depositLog.topics[1] // First indexed parameter
      const user = depositLog.topics[2] // Second indexed parameter
      
      // Parse the data field for non-indexed parameters
      const data = depositLog.data
      // Remove 0x prefix and parse as hex chunks of 32 bytes each
      ...
      return {
        depositId,
        user,
        amount: BigInt(chunks[0] || '0'),
        minReceive: BigInt(chunks[1] || '0'),
        feeCap: BigInt(chunks[2] || '0'),
        targetChainId: BigInt(chunks[3] || '0'),
        targetToken: chunks[4] ? '0x' + chunks[4].slice(26) : '0x0' // Remove padding
      }
    }
  }
  ...
  async executeRealSTXNBridge(
    amountUSDC: number,
    onProgress: (step: string, transaction?: WorkingAutomaticSTXNTransaction) => void
  ): Promise<WorkingAutomaticSTXNTransaction[]> {
    ...
    const transactions: WorkingAutomaticSTXNTransaction[] = []

    onProgress('🚀 Starting Working Automatic STXN Bridge: Base → Arbitrum')
    ...
    try {
      ...
      const depositTransaction: WorkingAutomaticSTXNTransaction = {
        hash: depositTx,
        chain: 'base',
        type: 'escrow_deposit',
      ...
      onProgress(`✅ Step 1 Complete: ${amountUSDC} USDC deposited! Deposit ID: ${depositId}`, depositTransaction)

      // STEP 2: Extract event info and provide cross-chain liquidity
      onProgress(`🔍 Step 2: Extracting deposit event and providing cross-chain liquidity...`)
      
      try {
        const depositInfo = await this.extractDepositInfo(depositTx)
        onProgress(`📋 Event extracted - Amount: ${(Number(depositInfo.amount) / 1e6).toFixed(6)} USDC, Min Receive: ${(Number(depositInfo.minReceive) / 1e6).toFixed(6)} USDC`)
        ...
        if (solverArbBalance < depositInfo.minReceive) {
          throw new Error(`Base solver has insufficient USDC on Arbitrum: ${(Number(solverArbBalance) / 1e6).toFixed(6)} < ${(Number(depositInfo.minReceive) / 1e6).toFixed(6)}`)
        }

        onProgress(`⚡ Providing ${(Number(depositInfo.minReceive) / 1e6).toFixed(6)} USDC liquidity: Base Solver → Arbitrum User`)
        
        // Provide liquidity: Base solver → Arbitrum user
        const liquidityTx = await this.baseSolverArbClient.writeContract({
          address: CONTRACTS.USDC_ARB,
          abi: erc20Abi,
          functionName: 'transfer',
          args: [ARB_USER_ADDRESS as `0x${string}`, depositInfo.minReceive]
        })

        const liquidityReceipt = await this.arbPublicClient.waitForTransactionReceipt({ hash: liquidityTx })
        
        const liquidityTransaction: WorkingAutomaticSTXNTransaction = {
          hash: liquidityTx,
          chain: 'arbitrum',
          type: 'cross_chain_liquidity',
          amount: (Number(depositInfo.minReceive) / 1e6).toFixed(6),
          status: 'success',
          timestamp: new Date(),
          ...
          description: `Base solver provided ${(Number(depositInfo.minReceive) / 1e6).toFixed(6)} USDC to Arbitrum user`,
          step: 2,
          depositId,
        ...
        onProgress(`✅ Step 2 Complete: Cross-chain liquidity provided!`, liquidityTransaction)

        // STEP 3: Check if solver is authorized and claim
        onProgress(`💎 Step 3: Checking authorization and claiming from escrow...`)
        ...
        if (!isAuthorized) {
          onProgress(`⚠️ Base solver not authorized - cannot claim from escrow`)
          ...
          const claimTransaction: WorkingAutomaticSTXNTransaction = {
            hash: claimTx,
            chain: 'base',
            type: 'solver_claim',
            amount: (Number(depositInfo.amount) / 1e6).toFixed(6),
            status: 'success',
            timestamp: new Date(),
            ...
            description: `Base solver claimed ${(Number(depositInfo.amount) / 1e6).toFixed(6)} USDC from escrow`,
            step: 3,
            depositId
          }
          
          transactions.push(claimTransaction)
          onProgress(`✅ Step 3 Complete: Solver claimed from escrow!`, claimTransaction)
        }

        onProgress(`🎉 Automatic STXN Bridge Complete! User received liquidity, solver processed payment!`)

      } catch (error: any) {
        onProgress(`❌ Event extraction or liquidity provision failed: ${error.message}`)
      }

      return transactions

    } catch (error: any) {
      onProgress(`❌ Working STXN Bridge failed: ${error.message}`)
      throw error
    }
  }
